...
/*从机地址：0b0110110*/
template <typename i2c_device_7bits> class AS5600 {
	...
	enum reg : std::uint8_t {
		...
		MAGNITUDE_H = 0x1C,
		MAGNITUDE_L =
			0x1B, /*展示内部的CORDIC(坐标旋转数字计算机)的幅度值，作用暂时不清楚*/
	...
	enum Hysteresis { DISBALE, LSB1, LSB2, LSB3 };
	enum output_mode {
		ANALOG1, /*GND~VDD(0%~100%)*/
		ANALOG2, /*GND~VDD(10%~90%)*/
		DIGITAL_PWM
	};
	enum PWM_frequency { _115Hz, _230Hz, _460Hz, _920Hz };
	enum slow_filter { _16x, _8x, _4x, _2x };
	...
	void
	set_fast_filter_threshold(fast_filter_threshold thresholds) // 12:10
	{
		switch (thresholds) {
		case ONLY_SLOW:
	...
	{
		enable ? BIT::SET(config_register_, 13) :
			 BIT::CLR(config_register_, 13);
	}

    public:
	static std::uint16_t read_CORDIC_magnitude_value()
	{
		std::uint8_t read_buf[2];
		i2c_device_7bits::mem_read(MAGNITUDE_L,
					   i2c_device_7bits::MEMADD_SIZE_8BIT,
					   read_buf, 2, DEFAULT_TIMEOUT);
		return (read_buf[1] << 8) | read_buf[0];
	}
	static std::uint8_t read_AGC()
	{
		std::uint8_t read_buf;
		i2c_device_7bits::mem_read(AGC,
					   i2c_device_7bits::MEMADD_SIZE_8BIT,
					   &read_buf, 1, DEFAULT_TIMEOUT);
		return read_buf;
	}
	static std::uint16_t read_angle_raw()
	{
		std::uint8_t read_buf[2];
		i2c_device_7bits::mem_read(RAW_ANGLE_L,
					   i2c_device_7bits::MEMADD_SIZE_8BIT,
					   read_buf, 2, DEFAULT_TIMEOUT);
	}
	...
	{
		std::uint8_t read_buf[2];
		i2c_device_7bits::mem_read(ANGLE_L,
					   i2c_device_7bits::MEMADD_SIZE_8BIT,
					   read_buf, 2, DEFAULT_TIMEOUT);
	}
	...
	{
		std::uint8_t read_buf;
		i2c_device_7bits::mem_read(STATUS,
					   i2c_device_7bits::MEMADD_SIZE_8BIT,
					   &read_buf, 1, DEFAULT_TIMEOUT);
		return (!BIT::READ(read_buf, 3) /*磁铁太强*/) &&
		       (!BIT::READ(read_buf, 4) /*磁铁太弱*/) &&
		       BIT::READ(read_buf, 5) /*检测到磁铁*/;
	}
	static void set_range(std::uint16_t start_angle = 0,
			      std::uint16_t stop_angle = 0xFFF,
			      std::uint16_t max_angle = 0xFFF)
	{
		if (start_angle > 0xFFF || stop_angle > 0xFFF ||
		    max_angle > 0xFFF) {
			return;
		}
		std::uint8_t write_buf[2] = {
			static_cast<std::uint8_t>(start_angle),
			static_cast<std::uint8_t>(start_angle >> 8)
		};
		i2c_device_7bits::mem_write(ZPOS_L,
					    i2c_device_7bits::MEMADD_SIZE_8BIT,
					    write_buf, 2, DEFAULT_TIMEOUT);
		write_buf[0] = static_cast<std::uint8_t>(stop_angle);
		write_buf[1] = static_cast<std::uint8_t>(stop_angle >> 8);
		i2c_device_7bits::mem_write(MPOS_L,
					    i2c_device_7bits::MEMADD_SIZE_8BIT,
					    write_buf, 2, DEFAULT_TIMEOUT);
		write_buf[0] = static_cast<std::uint8_t>(max_angle);
		write_buf[1] = static_cast<std::uint8_t>(max_angle >> 8);
		i2c_device_7bits::mem_write(MANG_L,
					    i2c_device_7bits::MEMADD_SIZE_8BIT,
					    write_buf, 2, DEFAULT_TIMEOUT);
	}
	void conf(power_mode mode = NOM, Hysteresis hyst = DISBALE,
		  output_mode out_mode = ANALOG1,
		  PWM_frequency pwm_freq = _115Hz, slow_filter slow_f = _16x,
		  fast_filter_threshold fast_f = ONLY_SLOW,
		  bool watchdog = false)
	{
		set_power_mode(mode);
		set_hysteresis(hyst);
		...
		std::uint8_t write_buf[2] = {
			static_cast<std::uint8_t>(config_register_),
			static_cast<std::uint8_t>(config_register_ >> 8)
		};
		i2c_device_7bits::mem_write(CONF_L,
					    i2c_device_7bits::MEMADD_SIZE_8BIT,
					    write_buf, 2, DEFAULT_TIMEOUT);
	}
};
 ...
 *         所有方法均为 static, 无需实例化
 */
template <typename spi_device> struct GD25Q {
	enum commands {
		WRITE_DATA = 0x02,
	...
	static void wait_for_write_end()
	{
		...
		spi_device::select();
		spi_device::transmit_without_ctl_select(&read_status_register,
							1);
		do {
	...
	static std::uint32_t read_id()
	{
		...
		std::uint8_t buf[4] = { READ_ID, DUMMY_BYTE, DUMMY_BYTE,
					DUMMY_BYTE };
	...
	static void page_write(std::uint8_t *pbuffer, std::uint32_t write_addr,
			       std::uint16_t num_byte_to_write)
	{
		write_enable();
		std::uint8_t send_buf[4] = {
			WRITE_DATA, static_cast<std::uint8_t>(write_addr >> 16),
		...
		spi_device::transmit_without_ctl_select(pbuffer,
							num_byte_to_write);
		spi_device::deselect();
		wait_for_write_end();
	}
	static void block_write(std::uint8_t *pbuffer, std::uint32_t write_addr,
				std::uint16_t num_byte_to_write)
	{
		std::uint8_t num_of_page = 0, num_of_single = 0, addr = 0,
			     count = 0, temp = 0;
	...
	static void buffer_read(std::uint8_t *pbuffer, std::uint32_t read_addr,
				std::uint16_t num_byte_to_read)
	{
		// 命令+地址 和 数据 需要在同一个 CS 窗口内，因此手动控 CS
		std::uint8_t send_buf[4] = {
			READ_DATA, static_cast<std::uint8_t>(read_addr >> 16),
			static_cast<std::uint8_t>(read_addr >> 8),
			static_cast<std::uint8_t>(read_addr >> 0)
		};
		spi_device::select();
		spi_device::transmit_without_ctl_select(send_buf, 4);
		spi_device::receive_without_ctl_select(pbuffer,
						       num_byte_to_read);
		spi_device::deselect();
	}
};

...
template <TimeProviderFunc GetTime, TaskConfig FirstTimer,
	  TaskConfig... RemainingTimers>
class StaticTimerManager {
    private:
	// 将所有传入的元组静态组合成一个 std::tuple 数组
	// 这里的类型是完全确定的
	static constexpr std::size_t TimerCount =
		1 + sizeof...(RemainingTimers);
	static constexpr TaskConfig Timers[TimerCount] = { FirstTimer,
							   RemainingTimers... };

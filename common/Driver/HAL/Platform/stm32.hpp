...
template <TIM_HandleTypeDef *htim, uint32_t clock_frequency> struct TIM {
...
template <typename TIMtype, uint32_t channel_, uint32_t frequency_> struct PWM {
	...
	static void set_frequency(uint32_t frequency)
	{
		if (frequency > 0 && frequency <= 1000000) {
			...
			__HAL_TIM_SET_PRESCALER(
				TIMtype::get_handle(),
				(TIMtype::get_clock_frequency() / 1000000.0f) -
					1);
			__HAL_TIM_SET_AUTORELOAD(TIMtype::get_handle(),
						 1000000.0f / frequency - 1);
		}
	}
	static void start()
	{
...
struct I2C_device_7bits {
	enum I2C_MEM_SIZE_T { MEMADD_SIZE_8BIT, MEMADD_SIZE_16BIT };
	static void transmit(uint8_t *pData, uint16_t Size, uint32_t Timeout)
	{
		HAL_I2C_Master_Transmit(hi2c, salve_address, pData, Size,
					Timeout);
	}
	static void receive(uint8_t *pData, uint16_t Size, uint32_t Timeout)
	{
		HAL_I2C_Master_Receive(hi2c, salve_address, pData, Size,
				       Timeout);
	}
	static void mem_write(uint16_t MemAddress, I2C_MEM_SIZE_T MemAddSize,
			      uint8_t *pData, uint16_t Size, uint32_t Timeout)
	{
		HAL_I2C_Mem_Write(hi2c, salve_address, MemAddress, MemAddSize,
				  pData, Size, Timeout);
	}
	static void mem_read(uint16_t MemAddress, I2C_MEM_SIZE_T MemAddSize,
			     uint8_t *pData, uint16_t Size, uint32_t Timeout)
	{
		HAL_I2C_Mem_Read(hi2c, salve_address, MemAddress, MemAddSize,
				 pData, Size, Timeout);
	}
};
#endif // HAL_I2C_MODULE_ENABLED
#ifdef HAL_UART_MODULE_ENABLED
template <UART_HandleTypeDef *huart> struct UART {
...
#endif
template <std::uint32_t USARTx> struct UART_LL {
	static void transmit(const uint8_t *pData, uint16_t Size,
			     uint32_t Timeout)
	{
		(void)Timeout;
		for (uint16_t i = 0; i < Size; ++i) {
			LL_USART_TransmitData8((USART_TypeDef *)USARTx,
					       *(pData + i));
			while (!(LL_USART_IsActiveFlag_TXE(

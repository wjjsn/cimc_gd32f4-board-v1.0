...
namespace BIT
{
template <typename T> inline void SET(T &var, int pos)
{
...
template <typename T> inline void CLR(T &var, int pos)
{
	static_assert(std::is_unsigned<T>::value,
		      "only works on unsigned integers");
	static_assert(sizeof(T) <= 8, "only supports up to 64-bit types");
...
template <typename T> inline void TGL(T &var, int pos)
{
	static_assert(std::is_unsigned<T>::value,
		      "only works on unsigned integers");
	static_assert(sizeof(T) <= 8, "only supports up to 64-bit types");
...
template <typename T> inline bool READ(T var, int pos)
{
	static_assert(std::is_unsigned<T>::value,
		      "only works on unsigned integers");
	static_assert(sizeof(T) <= 8, "only supports up to 64-bit types");

...
constexpr uint32_t DAC0_ADDR = DAC0;
}
...
template <uint32_t PERIPH> struct RCU_periph {
	static constexpr rcu_periph_enum periph = [] {
		if constexpr (PERIPH == registers::GPIOA_ADDR)
			...
			return RCU_ADC2;
		else if constexpr (PERIPH == registers::DAC0_ADDR)
			return RCU_DAC;
		else
			return []<bool b = false>() {
				static_assert(b, "Unsupported PERIPH");
				return 0;
			}();
	}();
};
...
/**
		...
		*/
template <uint8_t OTYPE, uint32_t SPEED, bit_status VAL> struct OutputConfig {
...
/**
		...
		*/
template <uint32_t AF_NUM> struct AFConfig {
...
/**
		...
		*/
template <uint32_t GPIOx, uint16_t Pin, uint32_t Mode, uint32_t PULL,
	  typename config>
struct GPIO {
...
/**
		...
		* @param  VAL: 初始值 (RESET/SET)
		*/
template <uint32_t GPIOx, uint16_t Pin, uint32_t PULL, uint8_t OTYPE,
	  uint32_t SPEED, bit_status VAL>
struct GPIO<GPIOx, Pin, GPIO_MODE_OUTPUT, PULL,
	    OutputConfig<OTYPE, SPEED, VAL> > {
	static void init()
	{
...
/**
		 ...
		 */
template <typename GPIO_SDA, typename GPIO_SCL, uint32_t I2Cx,
	  uint32_t clkspeed, uint8_t own_address7 = 0x00>
requires(GPIO_SDA::af_config::af_num == GPIO_AF_4 &&
	 GPIO_SCL::af_config::af_num == GPIO_AF_4) struct I2C_bus {
	...
	static void init()
	{
		...
		i2c_mode_addr_config(I2Cx, I2C_I2CMODE_ENABLE,
				     I2C_ADDFORMAT_7BITS, own_address7);
		i2c_enable(I2Cx);
		i2c_ack_config(I2Cx, I2C_ACK_ENABLE);
	}

	static void transmit(uint8_t slave_addr, uint8_t *p, uint16_t n,
			     uint32_t t)
	{
		(void)t;
	...
	static void receive(uint8_t slave_addr, uint8_t *p, uint16_t n,
			    uint32_t t)
	{
		(void)t;
		if (n == 0)
...
/**
		 ...
		 */
template <typename bus_t, uint8_t slave_address> struct I2C_device_addr {
	static void init()
	{
		bus_t::init();
	}
...
template <typename GPIO_TX, typename GPIO_RX, uint32_t USARTx, uint32_t baudval,
	  uint32_t paritycfg = USART_PM_NONE, uint32_t wlen = USART_WL_8BIT,
	  uint32_t stblen = USART_STB_1BIT>
requires((GPIO_TX::af_config::af_num == GPIO_AF_7 ||
	  GPIO_TX::af_config::af_num == GPIO_AF_8) &&
	 (GPIO_RX::af_config::af_num == GPIO_AF_7 ||
	  GPIO_RX::af_config::af_num == GPIO_AF_8)) struct USART_Device {
	...
	static void transmit(const uint8_t *pData, uint16_t Size,
			     uint32_t Timeout = 0)
	{
		(void)Timeout;
...
template <typename GPIO_MOSI, typename GPIO_MISO, typename GPIO_SCLK,
	  typename SPI_CONFIG>
requires(GPIO_MOSI::af_config::af_num == GPIO_AF_5 &&
	 GPIO_MISO::af_config::af_num == GPIO_AF_5 &&
	 GPIO_SCLK::af_config::af_num == GPIO_AF_5) struct SPI {
	...
	static void init()
	{
		...
		rcu_periph_clock_enable(
			RCU_periph<SPI_CONFIG::spi_periph>::periph);

		spi_parameter_struct spi_init_struct = {
			SPI_CONFIG::mode,	   SPI_CONFIG::transmode_val,
			SPI_CONFIG::framesize_val, SPI_CONFIG::nss_mode,
			SPI_CONFIG::endian_val,	   SPI_CONFIG::ckp,
...
/**
		 ...
		 */
template <typename bus_t, typename GPIO_CS> struct SPI_device {
	static void init()
	{
		bus_t::init();
		GPIO_CS::init();
	...
	static void transmit_without_ctl_select(uint8_t *p, uint16_t n,
						uint32_t t = 0)
	{
		bus_t::transmit(p, n, t);
	}

	static void receive_without_ctl_select(uint8_t *p, uint16_t n,
					       uint32_t t = 0)
	{
		bus_t::receive(p, n, t);
	}

	static void transfer_without_ctl_select(uint8_t *p, uint16_t n,
						uint32_t t = 0)
	{
		bus_t::transfer(p, n, t);
	}
...
template <uint32_t ADCx, uint32_t prescaler = ADC_ADCCK_PCLK2_DIV4,
	  uint32_t resolution = ADC_RESOLUTION_12B, uint32_t channel_num = 1,
...
/**
		...
		* @note   ADC频率不能超过40MHz
		*/
template <typename ADC_CONFIG> struct ADC {
	static void init()
	{
		rcu_periph_clock_enable(
			RCU_periph<ADC_CONFIG::adc_periph>::periph);
		adc_clock_config(ADC_CONFIG::prescaler_val);
		adc_sync_mode_config(ADC_SYNC_MODE_INDEPENDENT);
		adc_special_function_config(ADC_CONFIG::prescaler_val,
					    ADC_SCAN_MODE, ENABLE);
		adc_data_alignment_config(ADC_CONFIG::adc_periph,
					  ADC_DATAALIGN_RIGHT);
		adc_resolution_config(ADC_CONFIG::adc_periph,
				      ADC_CONFIG::resolution_val);
		adc_channel_length_config(ADC_CONFIG::adc_periph,
					  ADC_ROUTINE_CHANNEL,
					  ADC_CONFIG::channel_num_val);
		adc_external_trigger_config(ADC_CONFIG::adc_periph,
					    ADC_ROUTINE_CHANNEL,
					    ADC_CONFIG::trigger_val);
		adc_enable(ADC_CONFIG::adc_periph);
		adc_calibration_enable(ADC_CONFIG::adc_periph);
	}

	static void set_channel(uint8_t channel,
				uint8_t sample_time = ADC_SAMPLETIME_144)
	{
		// rank = 0: 写入 RSQ2[4:0] (第 0 次转换的通道号).
		// 用户手册 14.7.10: "单次运行模式下, ADC_RSQ2寄存器的RSQ0[4:0]位规定了ADC的转换通道".
		adc_routine_channel_config(ADC_CONFIG::adc_periph, 0, channel,
					   sample_time);
	}

	static uint16_t get_value()
	{
		adc_software_trigger_enable(ADC_CONFIG::adc_periph,
					    ADC_ROUTINE_CHANNEL);
		while (!adc_flag_get(ADC_CONFIG::adc_periph, ADC_FLAG_EOC))
			;
		uint16_t value = adc_routine_data_read(ADC_CONFIG::adc_periph);
		adc_flag_clear(
			ADC_CONFIG::adc_periph,
			ADC_FLAG_EOC); // 清标志, 防止下次 while 立即通过读到旧值
		return value;
	}
};
...
struct RTC_Device {
	...
	static void set_time(uint8_t year, uint8_t month, uint8_t date,
			     ...
			     uint8_t second)
	{
		...
		rtc_initpara.factor_syn =
			0xFF; // 256Hz   / (255+1) = 1Hz (秒脉冲)
	...
	static RTC_Time get_time()
	{
		...
		return RTC_Time{ rtc_initpara.year,  rtc_initpara.month,
				 rtc_initpara.date,  rtc_initpara.day_of_week,
				 rtc_initpara.hour,  rtc_initpara.minute,
				 rtc_initpara.second };
	}
};
...
/**
		...
		* @note   预分频设置为100ns分辨率
		*/
template <uint32_t TIMx> struct SysTick_Delay {
	static void init()
	{
...
template <uint32_t TIMx, uint32_t clock_frequency,
	  uint32_t psc_mul = RCU_TIMER_PSC_MUL4>
struct TIM {
	...
	static void init(uint16_t prescaler, uint32_t autoreload)
	{
		...
		timer_parameter_struct param = {
			prescaler,	  TIMER_COUNTER_EDGE, TIMER_COUNTER_UP,
			TIMER_CKDIV_DIV1, autoreload,	      0
		};
...
/**
		...
		*/
template <typename TIMtype, typename PWM_CONFIG> struct PWM {
	static void init()
	{
		TIMtype::init(TIMtype::get_clock_frequency() / 1000000.0f - 1,
			      1000000.0f / PWM_CONFIG::frequency_val - 1);

		timer_oc_parameter_struct oc_param = {
			TIMER_CCX_ENABLE,	    TIMER_CCXN_DISABLE,
			PWM_CONFIG::polarity_val,   PWM_CONFIG::npolity_val,
			PWM_CONFIG::idle_state_val, PWM_CONFIG::nidle_state_val
		};
		timer_channel_output_config(TIMtype::get_handle(),
					    PWM_CONFIG::channel_val, &oc_param);
		timer_channel_output_mode_config(TIMtype::get_handle(),
						 PWM_CONFIG::channel_val,
						 PWM_CONFIG::mode_val);
		timer_channel_output_shadow_config(TIMtype::get_handle(),
						   PWM_CONFIG::channel_val,
						   PWM_CONFIG::shadow_val);
	}

	static void set_compare(uint32_t compare)
	{
		timer_channel_output_pulse_value_config(TIMtype::get_handle(),
							PWM_CONFIG::channel_val,
							compare);
	}
	...
	static void start()
	{
		timer_channel_output_state_config(TIMtype::get_handle(),
						  PWM_CONFIG::channel_val,
						  TIMER_CCX_ENABLE);
	}
...
/**
		* @brief  DAC配置参数
		* @param  DACx: DAC外设 (DAC0_ADDR)
		* @param  out: 输出通道 (DAC_OUT0/DAC_OUT1)
		* @param  trigger: 触发源 (DAC_TRIGGER_SOFTWARE等)
		* @param  wave: 波形模式 (DAC_WAVE_DISABLE等)
		* @param  align: 数据对齐 (DAC_ALIGN_12B_R/12B_L/8B_R)
		*/
template <uint32_t DACx, uint8_t out = DAC_OUT0,
	  uint32_t trigger = DAC_TRIGGER_SOFTWARE,
	  uint32_t wave = DAC_WAVE_DISABLE, uint32_t align = DAC_ALIGN_12B_R>
struct DAC_config {
	static constexpr uint32_t dac_periph = DACx;
	static constexpr uint8_t out_val = out;
	static constexpr uint32_t trigger_val = trigger;
	static constexpr uint32_t wave_val = wave;
	static constexpr uint32_t align_val = align;
};

/**
		* @brief  DAC数模转换器
		* @param  DAC_CONFIG: DAC_config配置模板
		* @note   输出引脚需配为 GPIO_MODE_ANALOG
		*/
template <typename DAC_CONFIG> struct DAC {
	static void init()
	{
		rcu_periph_clock_enable(
			RCU_periph<DAC_CONFIG::dac_periph>::periph);
		dac_deinit(DAC_CONFIG::dac_periph);
		dac_trigger_source_config(DAC_CONFIG::dac_periph,
					  DAC_CONFIG::out_val,
					  DAC_CONFIG::trigger_val);
		dac_trigger_enable(DAC_CONFIG::dac_periph,
				   DAC_CONFIG::out_val);
		dac_wave_mode_config(DAC_CONFIG::dac_periph,
				     DAC_CONFIG::out_val,
				     DAC_CONFIG::wave_val);
		dac_output_buffer_enable(DAC_CONFIG::dac_periph,
					 DAC_CONFIG::out_val);
		dac_enable(DAC_CONFIG::dac_periph, DAC_CONFIG::out_val);
	}
	...
	static void set(uint16_t value)
	{
		if (value > 4095)
			value = 4095;
		dac_data_set(DAC_CONFIG::dac_periph, DAC_CONFIG::out_val,
			     DAC_CONFIG::align_val, value);
	}

	/// 软件触发一次转换
	static void trigger()
	{
		dac_software_trigger_enable(DAC_CONFIG::dac_periph,
					   DAC_CONFIG::out_val);
	}
};}
}

...
template <typename RingBuffer>
void Parser<RingBuffer>::frame_to_ascii(const uint8_t *binary,
					uint16_t binary_size, char *ascii_out,
					uint16_t &ascii_size)
{
...
template <typename RingBuffer> bool Parser<RingBuffer>::is_hex(char c)
{
...
template <typename RingBuffer> bool Parser<RingBuffer>::push_ascii(char c)
{
...
Protocol::Status Parser<RingBuffer>::decode_and_check(Frame &out_frame)
{
	...
	for (uint16_t i = 0; i < binary_len; ++i) {
		if (!hex_pair_to_byte(ascii_buf_[i * 2], ascii_buf_[i * 2 + 1],
				      binary[i])) {
			return Status::invalid_hex;
		}
	}
	...
	uint16_t tail = (static_cast<uint16_t>(binary[binary_len - 2]) << 8) |
			binary[binary_len - 1];
	...
	uint16_t crc_range_len =
		9 +
		content_len; // 帧头(2)+ID(2)+类型(1)+命令字(2)+长度(1)+版本(1)+内容(N)
	uint16_t calculated_crc =
		HAL::gd32f4::CRC16_MODBUS::calculate(binary, crc_range_len);

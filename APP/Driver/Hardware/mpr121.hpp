...
template <typename i2c_device_7bits> class mpr121 {
	...
	static void writeRegister(uint8_t reg, uint8_t value)
	{
		...
		i2c_device_7bits::mem_read(reg,
					   i2c_device_7bits::MEMADD_SIZE_8BIT,
					   &ecr_backup, 1, DEFAULT_TIMEOUT);
		...
		{
			i2c_device_7bits::mem_write(
				ECR, i2c_device_7bits::MEMADD_SIZE_8BIT,
				&ecr_chear, 1, DEFAULT_TIMEOUT);
		}
		i2c_device_7bits::mem_write(
			reg, i2c_device_7bits::MEMADD_SIZE_8BIT, &value, 1,
			DEFAULT_TIMEOUT); // 开始写入目标寄存器
	...
	static void init(uint8_t touch = 12, uint8_t release = 6)
	{
		...
		std::uint8_t c;
		i2c_device_7bits::mem_read(CONFIG2,
					   i2c_device_7bits::MEMADD_SIZE_8BIT,
					   &c, 1, DEFAULT_TIMEOUT);
		if (c != 0x24) {
			return;
		...
		uint8_t ECR_SETTING =
			0x8C; // 配置ECR寄存器，位的作用在数据手册的第16页
		writeRegister(ECR, ECR_SETTING); // start with above ECR setting
	}
	static uint16_t touched()
	{
		std::uint8_t buf[2];
		i2c_device_7bits::mem_read(TOUCHSTATUS_H,
					   i2c_device_7bits::MEMADD_SIZE_8BIT,
					   &buf[0], 1, DEFAULT_TIMEOUT);
		i2c_device_7bits::mem_read(TOUCHSTATUS_L,
					   i2c_device_7bits::MEMADD_SIZE_8BIT,
					   &buf[1], 1, DEFAULT_TIMEOUT);

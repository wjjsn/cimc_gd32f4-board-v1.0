...
{
	uint8_t buf[256];
	uint16_t sz = Response::build_response(device_id, cmd_word, content,
					       content_size, buf, sizeof(buf));
	if (sz)
		send_frame(buf, sz);
}
...
inline float read_ch2()
{
	...
	return voltage * voltage * -6.91f + 268.66f * voltage -
	       281.28f; // 二次线性；检测+-4度内
}
...
inline auto utc_to_rtc_bcd(uint32_t utc)
{
	...
	rtc.week = dec_to_bcd(wday); // 注：部分 RTC 的星期范围是
...
inline void cmd_0261_set_interval(const Frame &f)
{
	...
	if (frame_content_len(f) >= 1) {
		...
		if (code >= 1 && code <= 3) {
			...
			Params::save();
			g_auto_report_interval_ms = (code == 1) ? 1000 :
						    (code == 2) ? 3000 :
								  5000;
...
inline void cmd_0411_set_ch0_thr(const Frame &f)
{
	uint16_t devid = frame_devid(f);
	if (frame_content_len(f) >= 4) {
		Params::g_params.ch0_threshold =
			bytes_to_float(frame_content(f));
		Params::save();
...
inline void cmd_0602_get_alarms(const Frame &f)
{
	...
	for (int i = 0; i < Alarm::g_record_count; ++i) {
		if (Alarm::g_records[i].valid) {
			Alarm::format_record(Alarm::g_records[i], buf,
					     sizeof(buf));
			send_with_485(buf);
		}
	}
...
inline bool dispatch(const Frame &frame)
{
	...
	if (ftype == 0x01 || ftype == 0x05) {
		...
		case 0xFFFF: {
			// 广播寻址: 回复心跳
			uint8_t buf[64];
			uint16_t sz = Response::build_heartbeat(my_id, buf,
								sizeof(buf));
			if (sz)
				send_frame(buf, sz);
			return true;

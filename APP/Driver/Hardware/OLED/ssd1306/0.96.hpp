...
template <typename i2c_device_7bits> class OLED {
	...
	void show_image(std::uint8_t Page, std::uint8_t X, std::uint8_t Width,
			std::uint8_t Height, const std::uint8_t *Image)
	{
		for (int j = 0; j < Height; ++j) {
			for (int i = 1; i < Width + 1; ++i) {
				dataBuf1_[Page + j][X + i] =
					Image[Width * j + (i - 1)];
			}
		}
	}
	...
	void show_string(std::uint8_t Page, std::uint8_t X, char *String)
	{
		...
		while (String[i] != '\0') // 遍历字符串
		{
			...
			} else // 否则，即多字节字符
			{
				...
				for (pIndex = 0;
				     std::strcmp(OLED_CF16x16[pIndex].Index,
						 "") != 0;
				     pIndex++) {
					/*找到匹配的字符*/
					if (std::strcmp(
						    OLED_CF16x16[pIndex].Index,
						    SingleChar) == 0) {
						break; // 跳出循环，此时pIndex的值为指定字符的索引
					}
				}
				/*将字模库OLED_CF16x16的指定数据以16*16的图像格式显示*/
				show_image(Page, X + XOffset, 16, 2,
	...
	void init(void)
	{
		write_command(0xAE); // 设置显示开启/关闭，0xAE关闭，0xAF开启
		write_command(0xD5); // 设置显示时钟分频比/振荡器频率
		write_command(0x80); // 0x00~0xFF
		write_command(0xA8); // 设置多路复用率
		write_command(0x3F); // 0x0E~0x3F
		write_command(0xD3); // 设置显示偏移
		write_command(0x00); // 0x00~0x7F
		write_command(0x40); // 设置显示开始行，0x40~0x7F
	...
	{
		for (int i = 0; i < 8; ++i) {
			if (std::memcmp(&dataBuf1_[i][0], &dataBuf2_[i][0],
					129) != 0) {
				set_cursor(i, 0);
				i2c_device_7bits::transmit(
					&dataBuf1_[i][0], 129, DEFAULT_TIMEOUT);
				std::memcpy(&dataBuf2_[i][0], &dataBuf1_[i][0],
					    129);
			}
		}
	}
	void update_force(void)
	{
		for (int i = 0; i < 8; ++i) {
			set_cursor(i, 0);
			i2c_device_7bits::transmit(&dataBuf1_[i][0], 129,
						   DEFAULT_TIMEOUT);

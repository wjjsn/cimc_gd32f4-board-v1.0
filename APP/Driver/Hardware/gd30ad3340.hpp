...
public:
    ...
    {
        if (polarity) BIT::SET(config_reg_, COMP_POL);
        else          BIT::CLR(config_reg_, COMP_POL);
        if (latch)    BIT::SET(config_reg_, COMP_LAT);
        else          BIT::CLR(config_reg_, COMP_LAT);
    }
     ...
     * @retval None
     */
    void init(MUX       mux  = MUX_AIN0_GND,
              PGA       pga  = PGA_2048,

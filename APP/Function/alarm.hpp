...
namespace Alarm
{
	...
	constexpr uint32_t ALARM_FLASH_OFFSET = 64; // sizeof(DeviceParams) 约 52 字节, 64 对齐
	...
	inline void init()
	{
		for (auto &r : g_records) r.valid = false;
		g_record_count = 0;
		g_active	   = false;
	}

	inline void add(uint32_t timestamp, uint8_t channel, float threshold, float actual)
	{
		...
		if (g_record_count < MAX_RECORDS) ++g_record_count;
	}
	...
	{
		// 写入记录数 + 所有记录
		FlashParam::write(ALARM_FLASH_OFFSET, &g_record_count, sizeof(g_record_count));
	...
	{
		FlashParam::read(ALARM_FLASH_OFFSET, &g_record_count, sizeof(g_record_count));
		if (g_record_count > MAX_RECORDS) g_record_count = 0;

...
#ifdef GPIOA_BASE
template <uint32_t GPIO_Pin> using PA = GPIO<GPIOA_BASE, GPIO_Pin>;
#endif // GPIOA_BASE
#ifdef GPIOB_BASE
template <uint32_t GPIO_Pin> using PB = GPIO<GPIOB_BASE, GPIO_Pin>;
#endif // GPIOB_BASE

template <uint32_t tim_base, uint32_t clock_frequency> struct TIM {
...
struct PWM {
	static void set_compare(uint32_t compare)
	{
		...
		DL_Timer_setCaptureCompareValue(
			(GPTIMER_Regs *)TIMtype::get_handle(), compare,
			channel_);
	}

...
{
	uint8_t buf[64];
	uint16_t sz = Protocol::Response::build_ok(devid, cmd, buf, sizeof(buf));
...
static uint32_t addr_to_sector(uint32_t addr)
{
	...
	if (addr < 0x08008000) return CTL_SECTOR_NUMBER_1; // 16KB
	if (addr < 0x0800C000) return CTL_SECTOR_NUMBER_2; // 16KB
	if (addr < 0x08010000) return CTL_SECTOR_NUMBER_3; // 16KB
	if (addr < 0x08020000) return CTL_SECTOR_NUMBER_4; // 64KB
	if (addr < 0x08040000) return CTL_SECTOR_NUMBER_5; // 128KB
	if (addr < 0x08060000) return CTL_SECTOR_NUMBER_6; // 128KB
...
__attribute__((naked, noreturn)) static void jump_to_app_entry(uint32_t app_sp, uint32_t app_pc)
{
	__asm__ volatile(
		"msr msp, r0\n"
		"bx r1\n");
...
static void jump_to_app()
{
	...
	if (app_sp < 0x20000000 || app_sp > 0x20030000 || (app_sp & 0x7) != 0 ||
		app_pc < APP_START_ADDR || app_pc >= APP_START_ADDR + APP_SIZE || (app_pc & 0x1) == 0)
	{
...
static void run_upgrade_mode()
{
	...
	{
		char msg[64];
		snprintf(msg, sizeof(msg), "wait for start Application(%ds)……\r\n", sec);
		send_with_485(msg);
		uint32_t dl = systick_tick_ms + 1000;
		while (systick_tick_ms < dl)
		{
			Protocol::Frame f;
			if (poll_frame(f) && f.size >= 8 && f.data[4] == 0x01 && read_u16(&f.data[5]) == 0x0502)
			{
				started = true;
				goto recv_fw;
				}
...
recv_fw:
			...
			if (total < STAGING_SIZE)
			{
				...
				if (spos >= SLICE_SIZE)
				{
					for (uint32_t i = 0; i < SLICE_SIZE; i += 4)
						flash_write_word(STAGING_ADDR + total + i,
							sbuf[i] | ((uint32_t)sbuf[i+1] << 8) | ((uint32_t)sbuf[i+2] << 16) | ((uint32_t)sbuf[i+3] << 24));
					total += SLICE_SIZE; spos = 0;
				}
				lrt = systick_tick_ms;
			}
		}
		if ((systick_tick_ms - lrt > 5000 && total > 0) || total >= STAGING_SIZE) break;
	}
	...
	if (*(volatile uint32_t *)STAGING_ADDR == MAGIC_WORD)
	{
		...
		{
			Protocol::Frame f;
			if (poll_frame(f) && f.size >= 8 && f.data[4] == 0x01 && read_u16(&f.data[5]) == 0x0503)
			{
				send_ok(read_u16(&f.data[2]), 0x0503);
				flash_erase_range(APP_START_ADDR, APP_SIZE);
				for (uint32_t off = 0; off < total; off += 4)
					flash_write_word(APP_START_ADDR + off, *(volatile uint32_t *)(STAGING_ADDR + off));
				break;
			}
		}
	}
	...
	{
		send_error(devid);
		while (1) { Protocol::Frame f; poll_frame(f); }
	}
...
extern "C"
{
	int main(void)
	{
		...
		*BOOTLOADER_FLAG_ADDR = 0;
		...
		if (up) run_upgrade_mode();
		else    delay_ms(BOOT_TIMEOUT_S * 1000);
	...
	void USART1_IRQHandler()
	{
		if (usart_interrupt_flag_get(HAL::gd32f4::registers::USART1_ADDR, USART_INT_FLAG_RBNE) == SET)
			uart1_buffer::write_byte(usart_data_receive(HAL::gd32f4::registers::USART1_ADDR));
	}

	void SysTick_Handler(void) { systick_tick_ms = systick_tick_ms + 1U; }
	void _exit(int) { while (1) __asm__ volatile("nop"); }
}
